...
#define  PORT_KEYDAT  0x0060 //键盘 鼠标 消息端口
#define  PIC_OCW2     0x20   //操作控制字 主8259A芯片
#define  PIC1_OCW2    0xA0   //操作控制字 从8259A芯片
...
void intHandlerFromC(char* esp) {
    ...
    io_out8(PIC_OCW2, 0x20);        //处理一次中断后，可以再次处理中断
    unsigned char data = 0;
...
#define  PORT_KEYSTA  0x0064 //鼠标电路对应端口
...
void init_keyboard(void) {
    wait_KBC_sendready();
    io_out8(PORT_KEYCMD, KEYCMD_WRITE_MODE); //键盘进入数据接受状态
    wait_KBC_sendready();
    io_out8(PORT_KEYDAT, KBC_MODE); //激活鼠标相关器件
    return;
}
